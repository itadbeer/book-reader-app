import db
from typing import List, Any


class Category:
    def __init__(self):
        self.table_name = "category"
        db.do_connect()
        self.create_table()

    @staticmethod
    def create_table():
        query = """
        CREATE TABLE IF NOT EXISTS category (id INT NOT NULL GENERATED BY DEFAULT AS IDENTITY ( START 1 ),
        CONSTRAINT category_id PRIMARY KEY (id), name TEXT NOT NULL);
        """
        db.cursor.execute(query)
        return True

    def add(self, name: str) -> bool:
        query = f"INSERT INTO {self.table_name} (name) VALUES (%s)"
        db.cursor.execute(query, (name,))
        return True

    def update(self, category_id: int, category_name: str) -> bool:
        query = f"UPDATE {self.table_name} SET name=%s WHERE id=%s"
        db.cursor.execute(query, (category_name, category_id))
        return True

    def delete(self, category_id: int):
        query = f"DELETE FROM {self.table_name} WHERE id=%s"
        db.cursor.execute(query, (category_id,))
        return True

    def get_all(self) -> List:
        """ Get all of categories and return as a List """
        query = f"SELECT * FROM {self.table_name}"
        try:
            categories: List[Any] = db.cursor.execute(query).fetchall()
        except db.Error:
            return []
        finally:
            db.disconnect()
        categories_array = []
        for category in categories:
            category_dict = {
                'id': category[0], 'name': category[1]
            }
            categories_array.append(category_dict)
        return categories_array

    def count(self) -> int:
        query = f"SELECT count(*) FROM {self.table_name}"
        count = db.cursor.execute(query).fetchone()[0]
        db.disconnect()
        return int(count)

    def exist(self, category_id: int) -> bool:
        query = f"SELECT COUNT (*) FROM category WHERE id=%s"
        count = db.cursor.execute(query, (category_id,)).fetchone()[0]
        db.disconnect()
        return int(count) > 0
